...
class ContinuousVariable(Variable):
    ...
    def __init__(
        ...
        super(ContinuousVariable, self).__init__(
            idx=idx, parents=parents, noise=noise, functions=functions
        )
        self.betas = betas
        self.continuous_mapper_func = self.__create_continuous_mapper_func(
            functions=functions, betas=betas
        )

        # Input parameter validation
        if len(functions) != len(self._get_continuous_parents()):
            raise ValueError(
                f"There must be one function for each continuous parent. Expected "
                f"{len(self._get_continuous_parents())}, but were {len(functions)}"
            )

    def __create_continuous_mapper_func(
        self, functions: List[Callable[..., float]], betas: List[float]
    ...
    def get_non_root_signal(self, df: pd.DataFrame) -> pd.Series:
        ...
        continuous_signal = (
            df[continuous_parent_idxs].apply(self.continuous_mapper_func, axis=1)
        ...
        discrete_signal = (
            df[discrete_parent_idxs].apply(sum, axis=1)
