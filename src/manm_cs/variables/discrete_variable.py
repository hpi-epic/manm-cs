...
class DiscreteVariable(Variable):
    ...
    def __create_logit_mapper_func(self) -> Callable[[pd.Series], int]:
        ...
        def func(parent_values: pd.Series):
            return np.sum(
                [
                    np.where(
                        np.random.multinomial(1, self.mult_logit_function(value=value))
                        == 1
                    )[0][0]
    ...
    def get_non_root_signal(self, df: pd.DataFrame) -> pd.Series:
        ...
        continuous_signal = (
            df[continuous_parent_idxs].apply(self.logit_mapper_func, axis=1)
    ...
    def sample(self, df: pd.DataFrame, num_observations: int) -> pd.Series:
        if self._is_root():
            ...
            signal = pd.Series(
                CustomDiscreteDistribution(props).sample(
                    num_observations=num_observations
                )
            )  # This will return a list of zeros
        else:
