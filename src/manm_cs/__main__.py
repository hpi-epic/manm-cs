...
def type_in_range(
    type_: Type, lower_bound: Optional[float], upper_bound: Optional[float]
) -> Callable[[str], Any]:
    def assert_float_in_range(x: str):
        ...
        if lower_bound is not None and x < lower_bound:
            raise argparse.ArgumentTypeError(
                f"{x} not in range [{lower_bound}, {upper_bound}]"
            )
...
def funcs(function_str: str):
    try:
        ...
        raise argparse.ArgumentTypeError(
            f"{function_str} has wrong format or not supported"
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--num_nodes",
        type=type_in_range(int, 1, None),
        required=True,
        help="Defines the number of nodes to be in the generated DAG.",
    )
    parser.add_argument(
        "--edge_density",
        type=type_in_range(float, 0.0, 1.0),
        required=True,
    ...
    parser.add_argument(
        ...
        type=type_in_range(float, 0.0, None),
        required=False,
        default=1.0,
    ...
    parser.add_argument(
        "--beta_lower_limit",
        type=type_in_range_exclusive(float, 0.0, None),
        required=False,
        default=0.5,
...
if __name__ == "__main__":
    ...
    dfs = graph.sample(
        num_observations=args.num_samples, num_processes=args.num_processes
    )
