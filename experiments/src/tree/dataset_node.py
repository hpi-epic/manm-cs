...
class DatasetNode(BaseNode):
        ...
        for retry_id in range(max_retries):
            graph = (
                ...
                .with_discrete_signal_to_noise_ratio(
                    self.config.discrete_signal_to_noise_ratio
                )
            ...
            if nx_graph.edges:
                return graph
        raise Exception(
            f"Retried {max_retries} times to generate a graph but no edge was generated."
        )
    ...
    def resolve_impl(self):
        ...
        DatabaseAdapter.instance().upload_dataset(
            table_name=self.table_name, data_path=self.data_path
        )

        dataset = BackendAdapter.BackendAdapter.instance().create_dataset(
            table_name=self.table_name
        )
        dataset_id = dataset["id"]

        BackendAdapter.BackendAdapter.instance().upload_ground_truth(
            dataset_id, self.graph_path
        )
